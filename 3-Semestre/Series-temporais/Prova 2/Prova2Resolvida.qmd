...
Rodando o teste ADF nos dados da temperatura com a primeira diferença, temos um resultado de p-valor `r round(stat_test$p.value, 3)` para o teste ADF e `r round(stat_test_diff_kpss$p.value, 3)` para o teste KPSS, ambos os testes agora indicam que a série após a primeira diferença se tornou estacionária.
...
model_arfima <- fracdiff::fracdiff(ts_temperatura)

# Estimando o coeficiente da diferença
summary(model_arfima)
```
```{r}
# Estimando o modelo ARFIMA
model_arfima <- arima(ts_temperatura, order = c(0, model_arfima$d, 2))

# Coeficientes do modelo
coef_model_arfima <- lmtest::coeftest(model_arfima)
coef_model_arfima
```
