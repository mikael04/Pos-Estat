...
# X1: temperatura
# X2: concentração
# Y: produção do processo químico
dados<- data.frame(
...
#Modelos ajustados           Estimativas dos parâmetros      QMres   R2    R2a
#                          bo     b1  	b2      b3
...
#Em uma análise preliminar, e dos resultados apresentados no Quadro I, ??? poss???vel observar que os modelos 5 e 6 (com intera??????o entre X1 e X2) apresentam QMres e R2 muito 	 
#similares aos modelos 3 e 4. Isso indica que a interação não estã trazendo uma contribui??????o 	 
#significativa e, portanto, não deve fazer sentido mantê-la no modelo. Os modelos 3 e 4 	 
#parecem ser as melhores opções dentre os modelos analisados, pois para ambos tem-se QMres pequeno e, ainda, R2 = 0,9621, o que significa que 96,21% da variabilidade total de Y 	 
#estaria sendo explicada pelas regressoras X1 e X2.. Modelo indicado: Y em X1 e X2. 	 
#
# c) Representação matricial
#
...
#i) ??o é o intercepto do plano com o eixo Y. Não apresenta interpreta??????o pr???tica nesse estudo. 	 
#ii)  ??1 = 0,87 indica um aumento esperado na produção do processo qu???mico de 0,87 unidades 	 
#a cada acréscimo de uma unidade na temperatura, mantida a concentra??????o fixa em 10 ou 	 
#outro valor possível. 	 
#iii)  ??2 = 2,424 indica um aumento esperado na produção do processo qu???mico de 2,424 	 
#unidades a cada acréscimo de uma unidade na concentração, mantida a temperatura fixa em 	 
#100 ou outro valor possível. 	 
#
# f) Estiamtiva da variância
#s2	= QMres = 36,5
# 	 
# g) Valores preditos
...
pred
#
# h) Testa as hipóteses dos parâmetros (conjuntamente)
summary(mod3)
#F-statistic: 215.9 on 2 and 17 DF, p-value: 8.234e-13 	 
#??Conclusção: os p-valores obtidos são pequenos o suficiente para que se conclua pela 	 
#rejeição das hipóteses nulas H0:  ??j = 0 para j = 1, 2. 	 
#
# i) Testa as hipóteses dos parâmetros (individualmente)
summary(mod3)
#
# j) Intervalo de Confiança de 95% para os coeficientes
confint(mod3)
#Conclusão: Como o valor zero não pertence a nenhum dos intervalos, h??? 	 
#evidências para rejeição de H0:  ??2j = 0 para j = 0, 1 e 2. 	 
#
# k) Intervalo de confiança para a resposta média de y em x1=80 e x2=10
...
#
# l) Obtenha os coeficientes de determinação simples e parcial.
#r2Y1 = 0,7382
#r2Y2 = 0,2238
...
anova(mod3)
anova(mod1)
#Assim, ao ser adicionada a regressora X1 ao modelo que não contém nenhuma 
#regressora, a SQres (que, nesse caso, é a SQtotal) é reduzida em 73,82% e, quando 
#X2 é adicionada ao modelo que contém X1, a SQres(X1) é reduzida em 85,5%.
#Analogamente, adicionar X1 ao modelo que contém X2, faz com que a SQres(X2) seja
#reduzida em 95,1%.
#
# m) Obtenha os coeficientes de correlação simples e parcial.
# basta tirar a raíz quadrada tdo coeficiente de determinação
#rY1 = 0,859
#rY2 = 0,473
#rY2.1 = 0,9246
#rY1.2 = 0,9752
#
# n) Obtenha os resíduos e faça uma análise gráfica dos mesmos. 	 
resid<- mod3$residuals 	 
resid 	 
# Resíduos versus preditos, sqrt{|resíduos|} versus preditos, Normal Q-Qplot 	 
#e distância de Cook 	 
par(mfrow=c(2,2))
...
cor(X1,X2)
#
# o) Medidas para detecção de multicolinearidade
...
#
# p) Obtenha o gráfico dos dados observados e do plano ajustado.
...
#
# q) Obtenha o gráfico do plano ajustado.
